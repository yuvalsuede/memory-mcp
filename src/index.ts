...
function createServer(projectDir: string): McpServer {
  ...
  server.tool(
    ...
    async ({ id }) => {
      ...
      if (ok) syncClaudeMd(store, projectDir);
...
function syncClaudeMd(store: MemoryStore, projectDir: string): void {
...
export function createSandboxServer(): McpServer {
  return createServer("/tmp/memory-mcp-sandbox");
}
